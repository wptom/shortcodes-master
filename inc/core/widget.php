...
class Sm_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array(
			'classname'   => 'shortcodes-master',
			'description' => __( 'Special Shortcodes Master widget', 'sm' )
		);
		$control_ops = array(
			'width'   => 300,
			'height'  => 350,
			'id_base' => 'shortcodes-master'
		);
		$this->WP_Widget( 'shortcodes-master', __( 'Shortcodes Master', 'sm' ), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		$defaults = array(
			'title'   => __( 'Shortcodes Master', 'sm' ),
			'content' => ''
		);
		$instance = wp_parse_args( ( array ) $instance, $defaults );
?>
				<p>
					<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'sm' ); ?></label>
